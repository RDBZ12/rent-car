...
class DocumentosModel extends Query
{
    ...
    public function registrarDoc(string $documento, int $estado)
    {
        $verficar = "SELECT * FROM documentos WHERE documento = '$documento'";
        $existe = $this->select($verficar);
        if (empty($existe)) {
            $sql = "INSERT INTO documentos(documento, estado) VALUES (?, ?)";
            $datos = array($documento, $estado);
    ...
    public function modificarDoc(string $documento, int $estado, int $id)
    {
        $sql = "UPDATE documentos SET documento = ?, estado = ? WHERE id = ?";
        $datos = array($documento, $estado, $id);
        $data = $this->save($sql, $datos);
        if ($data == 1) {
            $res = "modificado";
        } else {
