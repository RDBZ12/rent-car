...
class Usuarios extends Controller{
    ...
    public function listar()
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            $idRow = (int) $data[$i]['id'];
            $btnEdit = '<button class="btn btn-outline-primary btn-sm" type="button" onclick="btnEditarUser(' . $idRow . ');" title="Editar"><i class="fas fa-edit"></i></button>';
            $btnDes = '<button class="btn btn-outline-danger btn-sm" type="button" onclick="btnEliminarUser(' . $idRow . ');" title="Desactivar"><i class="fas fa-trash-alt"></i></button>';
            $btnAct = '<button class="btn btn-outline-success btn-sm" type="button" onclick="btnReingresarUser(' . $idRow . ');" title="Reactivar"><i class="fas fa-undo"></i></button>';
    ...
    public function validar()
    {
        ...
        if (empty($usuario) || empty($clave)) {
            ...
            if ($data) {
                ...
                $_SESSION['perfil'] = 'default.png'; 
                $_SESSION['rol'] = $data['perfil']; // Map 'perfil' column to session 'rol'
    ...
    public function registrar()
    {
        ...
        if (true) {
            ...
            $rol = isset($_POST['rol']) ? strClean($_POST['rol']) : 'Administrador';
            $id = strClean($_POST['id']);
            if (empty($usuario) || empty($nombre) || empty($apellido)) {
                $msg = array('msg' => 'Todos los campos son obligatorios', 'icono' => 'warning');
            } elseif (!validarUsuarioSistema($usuario)) {
                $msg = array('msg' => 'El usuario debe tener 3-50 caracteres (letras, números, . _ -).', 'icono' => 'warning');
            } elseif (!validarNombrePersona($nombre) || !validarNombrePersona($apellido)) {
                $msg = array('msg' => 'Nombre y apellido solo deben contener letras y espacios.', 'icono' => 'warning');
            } else {
                if ($id == "") {
                    if (!empty($clave) && !empty($confirmar)) {
                        if ($clave != $confirmar) {
                            ...
                            $data = $this->model->registrarUsuario($usuario, $nombre, $apellido, $clave, $rol);
                            if ($data == "ok") {
                        ...
                        $msg = array('msg' => 'La contraseña es requerida para nuevos usuarios', 'icono' => 'warning');
                    }
                } else {
                    if (!empty($clave)) {
                        // Si se proporcionó una clave, validar que coincida con la confirmación
                        if ($clave != $confirmar) {
                            $msg = array('msg' => 'Las contraseñas no coinciden', 'icono' => 'warning');
                            echo json_encode($msg, JSON_UNESCAPED_UNICODE);
                            die();
                        }
                        $data = $this->model->modificarUsuarioConClave($usuario, $nombre, $apellido, $clave, $rol, $id);
                    } else {
                        // Si no hay clave, actualizar solo los datos base
                        $data = $this->model->modificarUsuario($usuario, $nombre, $apellido, $rol, $id);
                    }
    ...
    public function eliminar(int $id)
    {
        ...
        $data = $this->model->accionUser('Inactivo', $id);
        if ($data == 1) {
    ...
    public function reingresar(int $id)
    {
        ...
        $data = $this->model->accionUser('Activo', $id);
        if ($data == 1) {
